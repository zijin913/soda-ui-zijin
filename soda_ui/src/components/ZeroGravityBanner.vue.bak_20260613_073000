<template>
  <Transition name="zg-slide">
    <div v-if="conn.zeroGravityActive" class="zg-banner phosphor">
      <span class="zg-led" />
      <span class="zg-label">ZERO-GRAVITY ACTIVE — ARMS ARE FREE TO MOVE</span>
      <span class="zg-hint">on the robot host, hand-pose the arms to home then press <kbd>q</kbd> / <kbd>ESC</kbd> in the recovery popup</span>
    </div>
  </Transition>
</template>
