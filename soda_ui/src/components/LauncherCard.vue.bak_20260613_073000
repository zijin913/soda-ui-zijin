...
$ python -m soda_launcher</pre>
      ...
      <template v-else>
        ...
        <section class="actions">
          ...
          <button class="btn-stop" :disabled="!canStop" @click="onStop">
...
async function onStop() {
  ...
  if (!confirm(
    isReal
      ? 'Stop everything and enter zero-gravity?\n\n' +
        'The arms will become free to move by hand. On the robot host an OpenCV ' +
        'popup will appear — hand-pose the arms back to home, then press q / ESC ' +
        'in that popup to shut the stack down.'
      : 'Stop the sim and kill all processes?',
  )) return;
  actionError.value = null;
  const r = await conn.stop();
  if (!r.ok) actionError.value = r.error || 'stop failed';
}
